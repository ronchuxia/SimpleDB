...
public class JoinOptimizer {
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        int card = 1;
        // some code goes here
        if (joinOp == Predicate.Op.EQUALS) {    // Equality Join
            if (t1pkey && t2pkey) {
            ...
            int maxCard = Math.max(card1, card2);
            card = Math.max((int) (0.3 * card1 * card2), maxCard);
        }
    ...
    public Vector<LogicalJoinNode> orderJoins(
        ...
        for (int i = 0; i < joins.size(); ++i) {
            ...
            for (Set<LogicalJoinNode> joinSet: subsets) {   // enumerate through all the subsets of size i + 1
                double bestCostSoFar = Double.POSITIVE_INFINITY;
                bestCostCard = new CostCard();
                for (LogicalJoinNode joinToRemove: joinSet) {   // enumerate through all the subsets of joinSet of size i
                ...
                pc.addPlan(joinSet, bestCostSoFar, bestCostCard.card, bestCostCard.plan);   // add the optimal plan for joinSet to the plan cache
            }
        }
