...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		// The base case of the recursion: the page is a leaf page
		// You do not need to confirm that it actually contains the provided key value f.
		// Instead of directly calling BufferPool.getPage() to get each internal page and leaf page,
		// we recommend calling the wrapper function we have provided, BTreeFile.getPage().
		// It works exactly like BufferPool.getPage(),
		// but takes an extra argument to track the list of dirty pages.
		// This function will be important for the next two exercises
		// in which you will actually update the data and therefore need to keep track of dirty pages.
		if (pid.pgcateg() == BTreePageId.LEAF) {
			return (BTreeLeafPage) getPage(tid, dirtypages, pid, perm);
		}

		// The internal case of the recursion: the page is a internal page
		// Every internal (non-leaf) page your findLeafPage() implementation visits
		// should be fetched with READ_ONLY permission.
		BTreeInternalPage page = (BTreeInternalPage) getPage(tid, dirtypages, pid, Permissions.READ_ONLY);
		BTreeInternalPageIterator iterator = (BTreeInternalPageIterator) page.iterator();
		...
		// Finding the left-most leaf page is useful for scanning the entire file.
		if (f == null) {
			return findLeafPage(tid, dirtypages, entry.getLeftChild(), perm, null);
		}
		// If the provided value is not null
		while (entry.getKey().compare(Op.LESS_THAN, f) && iterator.hasNext()) {
			entry = iterator.next();
		}
		// If key >= f
		...
		else {	// If key < f but iterator.hasNext() == false
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		// Create a new page, insert tuples into the new page and delete them from the old page
		// Whenever you create a new page, either because of splitting a page or creating a new root page,
		// call getEmptyPage() to get the new page.
		// This function is an abstraction which will allow us to reuse pages
		// that have been deleted due to merging (covered in the next section).
		...
		// Set the left sibling and right sibling of the old page, the new page, and the right sibling of the old page
		...
		// Get the parent page and copy the key up to the parent page
		Field newKey = reverseIterator.next().getField(keyField);
		BTreeEntry newEntry = new BTreeEntry(newKey, page.getId(), newPage.getId());
		BTreeInternalPage parentPage = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), newKey);
		parentPage.insertEntry(newEntry);

		// Set the parent pointer of the new page to the parent page
		newPage.setParentId(parentPage.getId());

		// Determine whether to return the old page or the new page
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		BTreeInternalPageReverseIterator reverseIterator = (BTreeInternalPageReverseIterator) page.reverseIterator();
		...
		// Set the parent pointer of the children of the new page to the new page
		updateParentPointers(tid, dirtypages, newPage);

		// Get the parent page and move the key up to the parent page
		BTreeEntry oldEntry = reverseIterator.next();
		Field newKey = oldEntry.getKey();
		page.deleteKeyAndRightChild(oldEntry);	// Remove the key from the old page
		BTreeEntry newEntry = new BTreeEntry(newKey, page.getId(), newPage.getId());
		BTreeInternalPage parentPage = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), newKey);
		parentPage.insertEntry(newEntry);	// Insert the key to the parent page
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		int numTuples = page.getNumTuples(), siblingNumTuples = sibling.getNumTuples();
		int numTuplesSteal = (numTuples + siblingNumTuples) / 2 - numTuples;
		Iterator<Tuple> iterator;
		if (isRightSibling) {
			iterator = new BTreeLeafPageIterator(sibling);
		}
		else {
			iterator = new BTreeLeafPageReverseIterator(sibling);
		}
		Tuple tuple = null;
		for (int i = 0; i < numTuplesSteal; ++i) {
			tuple = iterator.next();
			sibling.deleteTuple(tuple);
			page.insertTuple(tuple);
		}

		// Update the key in the parent page
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// some code goes here
		int numTuples = page.getNumEntries(), siblingNumTuples = leftSibling.getNumEntries();
		int numTuplesSteal = (numTuples + siblingNumTuples) / 2 - numTuples;
		BTreeInternalPageReverseIterator iterator = new BTreeInternalPageReverseIterator(leftSibling);
		Tuple tuple = null;

		// First, rotate the key in the parent page to the page
		Field parentKey = parentEntry.getKey();
		BTreeEntry siblingEntry = iterator.next();
		BTreeEntry pageEntry = page.iterator().next();
		page.insertEntry(new BTreeEntry(parentKey, siblingEntry.getRightChild(), pageEntry.getLeftChild()));

		// Second, rotate the keys in the sibling page to the page
		for (int i = 1; i < numTuplesSteal; ++i) {
			leftSibling.deleteKeyAndRightChild(siblingEntry);
			page.insertEntry(siblingEntry);
			siblingEntry = iterator.next();
		}

		// Finally, rotate the key in sibling page to the parent page
		leftSibling.deleteKeyAndRightChild(siblingEntry);
		parentEntry.setKey(siblingEntry.getKey());
		parent.updateEntry(parentEntry);

		// Update the parent pointer of the children of the page and the sibling page
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		int numTuples = page.getNumEntries(), siblingNumTuples = rightSibling.getNumEntries();
		int numTuplesSteal = (numTuples + siblingNumTuples) / 2 - numTuples;
		...
		page.insertEntry(new BTreeEntry(parentKey, pageEntry.getRightChild(), siblingEntry.getLeftChild()));
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		BTreeInternalPageReverseIterator leftIterator = new BTreeInternalPageReverseIterator(leftPage);
		// Pull down the corresponding key in the parent entry
		BTreeEntry entry = iterator.next(), leftEntry = leftIterator.next();
		BTreeEntry newEntry = new BTreeEntry(parentEntry.getKey(), leftEntry.getRightChild(), entry.getLeftChild());
		...
		// Update the parent pointers of the children in the entries that were move
